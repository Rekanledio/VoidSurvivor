...
- [x] M6.4 Boomerang (2026-08-16): BoomerangData : WeaponData (+ maxDistance 6 / outSpeed 8 / returnSpeed 10), Boomerang (auto-attack loop, nearest-target gives throw direction, single-flight rule — no new throw while one is active), BoomerangProjectile (Outbound: straight until maxDistance from spawn; Return: re-aims at the player's CURRENT world position every physics frame, destroyed within 0.5; per-target hit-once HashSet — each throw hits a given enemy at most once while it may hit different enemies outbound and inbound). BoomerangData (dmg 7 / cd 1.2 / range 8) + Boomerang.prefab + BoomerangProjectile.prefab (green ring). Verified: 37/37 probe checks PASS (data, slot-0 equip, auto throw + single-flight, outbound speed 8.00 exact, reaches maxDistance 6.24, hit-once HP 23, multi-target 2 hits, return re-aims at moved player, returned & destroyed, kill → EnemyKilled Killer == Player, pickups, regressions), 0 errors/warnings.
- [ ] M6.5 Arc Blade
