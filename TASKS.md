...
- [x] M6.3 Scatter Blaster (2026-08-16): ScatterBlasterData : WeaponData (+ projectileCount 5 / spreadAngle 45), ScatterBlaster (auto-attack loop, nearest-target for the fan center, fires N PulseProjectiles simultaneously in a deterministic uniform fan: center → target, symmetric ±11.25/±22.5, no random), reuses PulseProjectile. ScatterBlasterData (dmg 3 / cd 0.8 / range 7 — low rate, multi-pellet, fan) + ScatterBlaster.prefab; WeaponController.Owner now lazy-resolves on every access (fixes weapons instantiated before parenting losing the player source). Verified: 42/42 probe checks PASS (data, slot-0 equip, beyond-range silent, in-range hit 30→21, fan geometry center 0.00/±11.2/±22.5 symmetric even normalized, volley cooldown 0.801s, kill → EnemyKilled Killer == Player, destroy, pickups, regressions) + manual play (auto fan kills target), 0 errors/warnings.
- [ ] M6.4 Boomerang
