...
- M10.1 — Boss Base Framework: the EXISTING M8.3 Boss implementation already provides the full base framework — `BossData : EnemyData` (SO, static config), `BossAI` (pursuit + contact damage via CombatSystem, IPoolable OnSpawn/OnDespawn), WaveManager W10 boss encounter (SpawnBossNow once, no normal schedule), `OnEnemyKilled` matching the active boss → `BossDefeated` → `Victory`, `BossSpawned`/`BossDefeated` events, boss reuses the EnemySpawner shared ObjectPool (no second pool), IPoolable lifecycle keeps pool reuse clean. No production code changed in M10.1 (verified 29/29 probe); no skills/phases/UI/VFX/new drops (deferred until M10.2+).
- M10.2 — Boss Projectile Skill (COMPLETE): BossAI fires ONE PulseProjectile every SkillCooldown (3s) at the player's CURRENT position when alive + Playing + player alive + within SkillRange (10). Reuses the SHARED PulseProjectile ObjectPool (no second pool) and CombatSystem.ApplyDamage (Source = boss, Damage = runtime Stats.Damage, inherits WaveMultiplier); direction locked at spawn (no homing), lifetime 3s, single hit then Release. Boss prefab wires projectilePrefab = PulseProjectile.prefab. Implementation params 3s/6.0/10.0 (not design values).
- M10.3 — Boss Finalization & Regression (COMPLETE): final parameter confirmation (base 500/20/1.5/1.5/1.0, W10 ×1.45 → 725/29/2.175, skill 3s/6.0/3.0s/10.0 — kept as MVP values, **no production parameter changes required**) + full battle acceptance (spawn / pursuit / contact 29 / skill 1 projectile / direction-locked no-homing / hit / release / cooldown / death chain once each / Victory / No Wave11 / pool reuse clean with skill reset / cross-run 2× each) + M6/M7/M9 regression. M10_3BossFinalRegressionProbe 55/55 PASS, 3× Play/Stop 0/0. **M10 = COMPLETE / ACCEPTED.*
## UI — Complete Game Flow (M11, COMPLETE / ACCEPTED)
## Audio/VFX — Basic Presentation Feedback (M12, COMPLETE / ACCEPTED)
## Save — Local Persistence (M13, IN PROGRESS)
- M13.1 — Save Foundation (COMPLETE): `SaveData` ([Serializable]: masterVolume/sfxVolume/bestWave/bestLevel/bestGold, defaults 1/1/0/0/0) + `SaveManager` (persistence service, singleton on GameManager GO via GameBootstrap, DontDestroyOnLoad; Save()/Load()/HasSave()/DeleteSave(); JsonUtility.ToJson + File.WriteAllText to `Path.Combine(Application.persistentDataPath, "VoidSurvivorSave.json")`; missing/corrupt file falls back to defaults without crashing; no run data persisted). M13.1 establishes only the persistence foundation — M13.2 (settings) / M13.3 (best run record) will wire business logic through it.
- M13.2 — Settings Persistence (COMPLETE): `AudioManager` owns runtime Master/Sfx volume (private set); `SetMasterVolume/SetSfxVolume` clamp 0..1 and persist via SaveManager (read-modify-write keeps best* fields for M13.3); `Awake` LoadSettings applies persisted volumes (missing/corrupt save -> 1.0/1.0). GameBootstrap order: SaveManager BEFORE AudioManager (Awake order = AddComponent order). M13.3 (best run record) not yet wired.
- M13.3 — Best Run Record (COMPLETE): `BestRunRecorder` subscribes `GameStateChanged`; on To==GameOver/Victory reads final run data (WaveManager.CurrentWave / PlayerProgress.Level / CurrentGold — still intact at terminal state), SaveManager.Load -> max(best*) -> Save only on improvement; `_recordedThisRun` guard (once per run); Playing resets it. Flow: GameStateChanged -> GameOver/Victory -> BestRunRecorder -> SaveManager.Load -> max(best*) -> SaveManager.Save. No run data persisted, RunRestarter untouched, no second save system. M13.4 (integration & acceptance) not yet implemented.
- M13.4 — Save Integration & Acceptance (COMPLETE): verified the three layers (Foundation / Settings / Best Record) coexist in the single SaveData without overwriting each other (read-modify-write both directions), across real Play sessions (0.73/0.41+5/3/120 -> 0.55/0.25+8/6/300), with Restart/MainMenu bounds, corrupt-JSON fallback and M9-M12 regression. No production code changes (acceptance only). **M13 = COMPLETE / ACCEPTED.**
- M12.1 — Audio Foundation: `AudioManager` (persistent singleton on GameManager GO, one reusable AudioSource, PlayOneShot, Master/Sfx volume).
- M12.2 — Core Gameplay SFX: 9 procedural WAVs in `Assets/Resources/Audio/SFX/`; `SfxLibrary` (SfxType->AudioClip, Resources.Load) + `GameplaySfx` (event wiring: DamageApplied->Hit (0.05s cooldown), EnemyDied/Pickup/LevelUp/BossSpawn/BossDefeat/PlayerDied/Victory; UI click via `SfxLibrary.PlayUiClick` on panel buttons).
- M12.3 — Basic VFX Feedback: 8 ParticleSystem prefabs (Assets/Resources/VFX/, short-lived autoDestroy); `VFXManager` (event wiring to prefab spawn at event position; 0.05s DamageApplied cooldown; cached player ref).
- M12.4 — Integration & Acceptance: VFXManager runtime-safe (Resources.Load, no UnityEditor dependency); Editor probe 40/40 + Windows Standalone64 Build success (runtime init stable). All audio/vfx event-driven through EventBus; no gameplay system touches AudioSource/ParticleSystem directly.
- M11.1 — Main Menu: `MainMenuPanel` (title + Play/Quit), GameStateChanged-driven visibility, Play = MainMenu→Playing, Quit = Application.Quit (Editor: stop play).
- M11.2 — Gameplay HUD: `GameplayHUD` (HP/XP/Level/Gold/Wave texts), visible during Playing/LevelUp/Shop/Paused, hidden GameOver/Victory/MainMenu; reads cached PlayerHealth/PlayerProgress/WaveManager refs every frame (no second event system, no per-frame Find).
- M11.3 — Result Screens: `GameOverPanel` + `VictoryPanel` (one `ResultPanel` instance each, showInState), Restart via `RunRestarter.RestartRun()` (Core: PlayerProgress/PlayerStats/weapons/PlayerHealth ResetForRun + MainMenu→Playing; WaveManager auto-start wave 1), MainMenu via GameManager.
- M11.4 — Full UI Integration & Acceptance: `GameFlow` (Core) bridges PlayerDied → GameOver (Playing-only guard); non-gameplay states freeze gameplay simulation — `WeaponController.GameplayActive` (4 weapons) + `EnemyController.GameplayActive` (4 enemy AIs + BossAI movement/contact) guard new attacks while MainMenu/GameOver/Victory; Restart also revives the player (PlayerHealth.ResetForRun). Single Canvas/EventSystem/InputSystemUIInputModule throughout; UI state matrix verified 48/48.
# Void Survivor — Architecture
